...
def merchant_orders(restaurant_id: UUID, limit: int = 50, session: Session = Depends(get_session)):
    return OrderService(session).list_for_restaurant(restaurant_id, limit)


@router.get("/tables", response_model=List[TableRead])
def merchant_tables(restaurant_id: UUID, session: Session = Depends(get_session)):
...
def update_table_status(table_id: UUID, body: TableStatusUpdate, session: Session = Depends(get_session)):
...
def merchant_analytics(restaurant_id: UUID, session: Session = Depends(get_session)):
